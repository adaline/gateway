...
import { TogetherAIChatModelConfigs } from "../../../src/configs";
import { BaseChatModel } from "../../../src/models";
import { TogetherAIChatRequestType, TogetherAICompleteChatResponse } from "../../../src/models/chat-models/types";
...
describe("BaseChatModel", () => {
  ...
  describe("constructor", () => {
    it("should initialize properties correctly", () => {
      ...
      expect(baseChatModel.getDefaultBaseUrl()).toBe("https://api.together.xyz/v1");
    });
  });
  ...
  describe("getDefaultBaseUrl", () => {
    it("should return the baseUrl without trailing slash", () => {
      ...
      expect(model.getDefaultBaseUrl()).toBe("https://api.together.xyz/v1");
    });
  });
  ...
  describe("transformConfig", () => {
    ...
    it("should correctly transform logProbs false to 0", () => {
      ...
      expect(transformed.logprobs).toEqual(0);
    });
  ...
  describe("BaseChatModel transformMessages", () => {
    ...
    it("should correctly transform a simple System message", () => {
      ...
      const expected: TogetherAIChatRequestType = {
        messages: [
          {
  ...
  describe("transformStreamChatResponseChunk", () => {
    ...
    it("should throw ModelResponseError for invalid data structure (Zod schema fail)", async () => {
      // Valid JSON, but doesn't match TogetherAIStreamChatResponse schema (missing required fields)
  ...
  describe("transformCompleteChatResponse", () => {
    ...
    describe("transformCompleteChatResponse", () => {
      ...
      it("should transform a response with both content and refusal (unusual but handled)", () => {
        ...
        const expected: ChatResponseType = {
          messages: [
            {
              role: AssistantRoleLiteral,
              content: [{ modality: "text", value: "Here is some allowed content." }],
            },
          ],
          usage: {
            promptTokens: 20,
      ...
      it("should omit optional usage and logProbs if not present in response (though TogetherAI usually includes usage)", () => {
        ...
        const safeMinimal = TogetherAICompleteChatResponse.safeParse(responseMinimal);
        expect(safeMinimal.success).toBe(true);
          ...
          choices: [
            {
              ...
              message: {
                ...
                tool_calls: [
                  {
                    // Missing 'id' which is required by TogetherAICompleteChatResponse schema
                    type: "function",
