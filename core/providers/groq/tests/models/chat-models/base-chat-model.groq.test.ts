...
import { OpenAIChatModelConfigs, OpenAIChatRequestType, OpenAICompleteChatResponse } from "@adaline/openai";
...
import { BaseChatModelGroq } from "../../../src/models";
...
describe("BaseChatModelGroq", () => {
  ...
  describe("constructor", () => {
    it("should initialize properties correctly", () => {
      const BaseChatModel = new BaseChatModelGroq(mockModelSchema, mockOptions);
      expect(BaseChatModel.modelSchema).toBe(mockModelSchema);
      expect(BaseChatModel.getDefaultBaseUrl()).toBe("https://api.groq.com/openai/v1");
    });
  });

  describe("getDefaultBaseUrl", () => {
    it("should return the baseUrl without trailing slash", () => {
      const model = new BaseChatModelGroq(mockModelSchema, mockOptions);
      expect(model.getDefaultBaseUrl()).toBe("https://api.groq.com/openai/v1");
    });
  });
  ...
  describe("transformConfig", () => {
    let model: BaseChatModelGroq;
    let tools: ToolType[];
    let messages: MessageType[];

    beforeEach(() => {
      model = new BaseChatModelGroq(mockModelSchema, mockOptions);
      tools = [];
      messages = [];
    });
  ...
  describe("BaseChatModelGroq transformMessages", () => {
    let model: BaseChatModelGroq;
    ...
    it("should throw InvalidMessagesError if safeParse fails", () => {
      ...
      expect(() => model.transformMessages(invalidInput as any)).toThrowError();
    });
    ...
    it("should throw InvalidMessagesError for System role with non-text modality", () => {
      ...
      try {
        ...
        expect(e).toBeInstanceOf(InvalidMessagesError);
        expect(e.info).toContain("Invalid message content for model : 'test-model'");
        expect(e.cause?.message).toContain("Prompting with images is incompatible with system messages`)");
      }
    });
    ...
    it("should correctly transform an Assistant message with mixed text and tool calls", () => {
      ...
      const expected: OpenAIChatRequestType = {
        messages: [
          {
            role: "assistant", // "assistant role"
            content: "Okay, I will call those tools.",
    ...
    it("should throw error if system with image", () => {
      const messages: MessageType[] = [
      ...
      try {
        expect(model.transformMessages(messages));
      } catch (e: any) {
        expect(e).toBeInstanceOf(InvalidMessagesError);
        expect(e.info).toContain("Invalid message content for model : 'test-model'");
        expect(e.cause?.message).toContain("Prompting with images is incompatible with system messages`)");
    ...
    it("should correctly transform a sequence of various valid messages", () => {
      const messages: MessageType[] = [
        { role: UserRoleLiteral, content: [{ modality: TextModalityLiteral, value: "System prompt." }] },
        { role: UserRoleLiteral, content: [{ modality: TextModalityLiteral, value: "User query?" }] },
        {
          role: AssistantRoleLiteral,
          content: [
  ...
  describe("transformCompleteChatResponse", () => {
    let BaseChatModel: BaseChatModelGroq;

    beforeEach(() => {
      BaseChatModel = new BaseChatModelGroq(mockModelSchema, mockOptions);
    });
  ...
  describe("transformTools", () => {
    let modelWithTools: BaseChatModelGroq;
    let modelWithoutTools: BaseChatModelGroq;
    ...
    beforeEach(() => {
      ...
      modelWithTools = new BaseChatModelGroq(mockModelSchema, mockOptions);
      ...
      modelWithoutTools = new BaseChatModelGroq(schemaWithoutTools as any, mockOptions); // Cast as any if type complains
    });
