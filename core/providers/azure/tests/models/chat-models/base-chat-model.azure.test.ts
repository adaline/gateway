...
import { BaseChatModelOpenAI } from "../../../src/models";
...
describe("BaseChatModelOpenAI", () => {
  ...
  const mockOptions = {
    apiKey: "test-api-key",
    deploymentId: "test-model",
    resourceName: "test-resource",
  };

  describe("constructor", () => {
    it("should initialize properties correctly", () => {
      const baseChatModel = new BaseChatModelOpenAI(mockModelSchema, mockOptions);
      expect(baseChatModel.modelSchema).toBe(mockModelSchema);
      expect(baseChatModel.getDefaultBaseUrl()).toBe("https://test-resource.openai.azure.com/openai/deployments/test-model");
    });
  });

  describe("getDefaultBaseUrl", () => {
    it("should return the baseUrl without trailing slash", () => {
      const model = new BaseChatModelOpenAI(mockModelSchema, mockOptions);
      expect(model.getDefaultBaseUrl()).toBe("https://test-resource.openai.azure.com/openai/deployments/test-model");
    });

    it("should return the baseUrl without trailing slash when provided with one", () => {
      const modelWithTrailingSlash = new BaseChatModelOpenAI(mockModelSchema, {
        ...mockOptions,
        baseUrl: "https://api.example.com/example/v1",
      });
      expect(modelWithTrailingSlash.getDefaultBaseUrl()).toBe("https://api.example.com/example/v1/openai/deployments/test-model");
    });

    it("should return a custom baseUrl if provided", () => {
      const modelWithUrlPath = new BaseChatModelOpenAI(mockModelSchema, {
        ...mockOptions,
        baseUrl: "https://api.example.com/openai/v1",
      });
      expect(modelWithUrlPath.getDefaultBaseUrl()).toBe("https://api.example.com/openai/v1/openai/deployments/test-model");
    });
  });
  ...
  describe("BaseChatModel transformMessages", () => {
    ...
    it("should throw InvalidMessagesError for PDF modality (not supported by Azure)", () => {
      const messages: MessageType[] = [
        {
          role: UserRoleLiteral,
          content: [
  ...
  describe("transformTools", () => {
    ...
    beforeEach(() => {
      ...
      modelWithTools = new BaseChatModelOpenAI(mockModelSchema, mockOptions);
      ...
      modelWithoutTools = new BaseChatModelOpenAI(schemaWithoutTools as any, mockOptions); // Cast as any if type complains
    });
